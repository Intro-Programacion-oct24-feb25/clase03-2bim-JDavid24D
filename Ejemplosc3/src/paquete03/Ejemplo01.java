...
public class Ejemplo01 {
    ...
    public static void main(String[] args) {
        ...
        String[] promedioNotasCualitativa= new String[3];
        double promedio;
        String cadena;
        // 0 - 5.9 Bueno
        // 6 - 8.9 Muy Bueno
        ...
        for (int i = 0; i < notas.length; i++) { // 0<3
            ...
            if (promedio >=0 && promedio <= 5.9){
                promedioNotasCualitativa[i] = "Sobresaliente";
            }else {
                 if (promedio>=6 && promedio<=8.9){
                promedioNotasCualitativa[i] = "Muy Bueno";
            }else{  
                 if (promedio>=9 && promedio<=10){
                     promedioNotasCualitativa[i] = "Bueno";
                 }
               }
            }
        }
        ...
        for (int i = 0; i < nombresEstudiante.length; i++) {
            reporte = String.format("%sEstudiante %s tiene una sumataria "
                    + "de notas: %.3f y la nota de sus sumas cualitativas es : %s\n",
                    reporte,
                    nombresEstudiante[i],
                    sumaNotas[i],
                    promedioNotasCualitativa[i]);
        }
